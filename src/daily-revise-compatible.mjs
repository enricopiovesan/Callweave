...
const resultRef = ({ workspaceId, locationId, revisionRequestId, mappingPolicyId, idempotencyKey }) =>
  `daily-canvas-revision-${createHash('sha256').update(JSON.stringify({
    workspaceId,
    locationId,
    revisionRequestId,
    mappingPolicyId,
...
function validateRequest(request) {
  ...
  if (!request.daily_revision_request?.id || !request.visual_mapping_policy?.id) return 'invalid_input';
...
export function runDailyReviseCompatible({ request, runtime }) {
  const invalid = validateRequest(request);
  if (invalid) {
    return {
      status: 'rejected',
      reason_code: 'invalid_input',
      daily_canvas_revision_ref: null,
  ...
  const renderAuth = runtime.authorize({
    adapterId: 'local-object-store',
    operation: 'store-daily-canvas-revision',
    payloadBytes: JSON.stringify({
      daily_revision_request: request.daily_revision_request,
  ...
  const appendAuth = runtime.authorize({
    adapterId: 'local-state-store',
    operation: 'append-daily-canvas-revision',
    payloadBytes: JSON.stringify({
      workspace_id: workspaceId,
      location_id: locationId,
      daily_revision_request: request.daily_revision_request,
  ...
  return {
    status: 'completed',
    reason_code: 'ok',
    daily_canvas_revision_ref: resultRef({
      workspaceId,
      locationId,
      revisionRequestId: request.daily_revision_request.id,
