import { createHash } from 'node:crypto';

const traceId = (requestId, reasonCode) =>
  `trace-${createHash('sha256').update(JSON.stringify({ requestId, reasonCode })).digest('hex').slice(0, 16)}`;

const resultRef = ({ workspaceId, locationId, operationsRequestId, operationsPolicyId, idempotencyKey }) =>
  `operations-report-${createHash('sha256').update(JSON.stringify({
    workspaceId,
    locationId,
    operationsRequestId,
    operationsPolicyId,
    idempotencyKey,
  })).digest('hex').slice(0, 16)}`;

function validateRequest(request) {
  if (!request || typeof request !== 'object') return 'invalid_input';
  if (!request.request_id || !request.workspace_id || !request.location_id || !request.idempotency_key) return 'invalid_input';
  if (!request.operations_request?.id) return 'invalid_input';
  if (!request.operations_policy?.id) return 'invalid_input';
  const context = request.runtime_context;
  if (!context) return 'invalid_input';
  if (!['resolvable', 'unresolvable'].includes(context.input_reference_state)) return 'invalid_input';
  if (!['available', 'unavailable'].includes(context.dependency_state)) return 'invalid_input';
  if (!['allowed', 'denied'].includes(context.policy_state)) return 'invalid_input';
  return null;
}

export function runOperationsRecoverCompatible({ request, runtime }) {
  const invalid = validateRequest(request);
  if (invalid) {
    return {
      status: 'rejected',
      reason_code: 'invalid_input',
      operations_report_ref: null,
      trace_ref: traceId(request?.request_id ?? 'missing', 'invalid_input'),
      warnings: ['invalid_input'],
    };
  }

  const {
    request_id: requestId,
    workspace_id: workspaceId,
    location_id: locationId,
    idempotency_key: idempotencyKey,
  } = request;
  const context = request.runtime_context;

  if (context.input_reference_state === 'unresolvable') {
    return {
      status: 'rejected',
      reason_code: 'invalid_input',
      operations_report_ref: null,
      trace_ref: traceId(requestId, 'invalid_input'),
      warnings: ['invalid_input'],
    };
  }

  if (context.dependency_state === 'unavailable') {
    return {
      status: 'deferred',
      reason_code: 'dependency_unavailable',
      operations_report_ref: null,
      trace_ref: traceId(requestId, 'dependency_unavailable'),
      warnings: ['dependency_unavailable'],
    };
  }

  if (context.policy_state === 'denied') {
    return {
      status: 'rejected',
      reason_code: 'policy_denied',
      operations_report_ref: null,
      trace_ref: traceId(requestId, 'policy_denied'),
      warnings: ['policy_denied'],
    };
  }

  const stateStoreActivation = runtime.activate({
    adapterId: 'local-state-store',
    target: 'local',
    configRef: { id: `${workspaceId}:${locationId}:state-store` },
  });
  const exportActivation = runtime.activate({
    adapterId: 'operations-export',
    target: 'local',
    configRef: { id: `${workspaceId}:${locationId}:operations-export` },
  });
  const backupActivation = runtime.activate({
    adapterId: 'backup-destination',
    target: 'local',
    configRef: { id: `${workspaceId}:${locationId}:backup-destination` },
  });

  for (const activation of [stateStoreActivation, exportActivation, backupActivation]) {
    if (!activation.activated) {
      return {
        status: 'deferred',
        reason_code: 'dependency_unavailable',
        operations_report_ref: null,
        trace_ref: activation.trace.id,
        warnings: ['dependency_unavailable'],
      };
    }
  }

  const sharedPayload = JSON.stringify({
    operations_request: request.operations_request,
    operations_policy: request.operations_policy,
    idempotency_key: idempotencyKey,
  }).length;

  const appendAuth = runtime.authorize({
    adapterId: 'local-state-store',
    operation: 'append-operations-report',
    payloadBytes: sharedPayload,
  });
  const exportAuth = runtime.authorize({
    adapterId: 'operations-export',
    operation: 'export-governed-state',
    payloadBytes: sharedPayload,
  });
  const backupAuth = runtime.authorize({
    adapterId: 'backup-destination',
    operation: 'write-encrypted-backup',
    payloadBytes: sharedPayload,
  });

  for (const auth of [appendAuth, exportAuth, backupAuth]) {
    if (!auth.authorized) {
      return {
        status: 'deferred',
        reason_code: 'dependency_unavailable',
        operations_report_ref: null,
        trace_ref: auth.trace.id,
        warnings: ['dependency_unavailable'],
      };
    }
  }

  return {
    status: 'completed',
    reason_code: 'ok',
    operations_report_ref: resultRef({
      workspaceId,
      locationId,
      operationsRequestId: request.operations_request.id,
      operationsPolicyId: request.operations_policy.id,
      idempotencyKey,
    }),
    trace_ref: traceId(requestId, 'ok'),
    warnings: [],
  };
}
