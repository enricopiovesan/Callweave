...
const resultRef = ({ workspaceId, locationId, proposalRefId, approvalDecisionId, idempotencyKey }) =>
  `knowledge-version-${createHash('sha256').update(JSON.stringify({
    workspaceId,
    locationId,
    proposalRefId,
    approvalDecisionId,
...
function validateRequest(request) {
  ...
  if (!request.knowledge_proposal_ref?.id || !request.approval_decision?.id) return 'invalid_input';
...
export function runKnowledgeManageCompatible({ request, runtime }) {
  const invalid = validateRequest(request);
  if (invalid) {
    return {
      status: 'rejected',
      reason_code: 'invalid_input',
      knowledge_version_ref: null,
  ...
  const appendAuth = runtime.authorize({
    adapterId: 'local-state-store',
    operation: 'append-knowledge-version',
    payloadBytes: JSON.stringify({
      knowledge_proposal_ref: request.knowledge_proposal_ref,
  ...
  return {
    status: 'completed',
    reason_code: 'ok',
    knowledge_version_ref: resultRef({
      workspaceId,
      locationId,
      proposalRefId: request.knowledge_proposal_ref.id,
      approvalDecisionId: request.approval_decision.id,
      idempotencyKey,
    }),
