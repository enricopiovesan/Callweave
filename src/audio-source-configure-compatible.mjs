...
const resultRef = ({ workspaceId, locationId, audioSourceId, recordingProfileId, idempotencyKey }) =>
  `audio-source-${createHash('sha256').update(JSON.stringify({
    workspaceId,
    locationId,
    audioSourceId,
    recordingProfileId,
...
function validateRequest(request) {
  ...
  if (!request.audio_source?.id || !request.recording_profile?.id) return 'invalid_input';
...
/**
 * Thin host-owned compatible execution path for callweave.audio-source-configure.
 * It does not implement microphone business rules; it only enforces declared
 * adapter activation/authorization and maps governed runtime states to contract outputs.
 */
export function runAudioSourceConfigureCompatible({ request, runtime }) {
  ...
  const calibrationAuth = runtime.authorize({
    adapterId: 'audio-device',
    operation: 'configure-and-calibrate',
    payloadBytes: JSON.stringify({
      audio_source: request.audio_source,
      recording_profile: request.recording_profile,
      idempotency_key: idempotencyKey,
    }).length,
  });
  if (!calibrationAuth.authorized) {
    return {
      status: 'deferred',
      reason_code: 'dependency_unavailable',
      audio_source_ref: null,
      trace_ref: calibrationAuth.trace.id,
      warnings: ['dependency_unavailable'],
    };
  }
  ...
  return {
    status: 'completed',
    reason_code: 'ok',
    audio_source_ref: resultRef({
      workspaceId,
      locationId,
      audioSourceId: request.audio_source.id,
      recordingProfileId: request.recording_profile.id,
      idempotencyKey,
    }),
