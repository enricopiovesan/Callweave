...
const appRoot = join(repoRoot, 'apps', 'callweave-evidence-retain');
const componentDir = join(appRoot, 'components', 'evidence-retain');
const workflowDir = join(appRoot, 'workflows', 'evidence-retain');
const contractPath = join(repoRoot, 'traverse', 'contracts', 'callweave', 'evidence-retain', 'contract.json');
...
const wrapper = {
  ...
  capability_id: 'callweave.evidence-retain',
  notes: 'Thin wrapper only. Host-owned retention reference resolution, evidence persistence, and append-only lifecycle state remain outside the portable contract.',
};
...
const componentManifest = {
  component_id: 'callweave.evidence-retain-compatible-component',
  version: '0.1.0',
  ...
  validation_evidence: [
    {
      ...
      status: 'passed',
      produced_by: 'evidence_retain_compatible_smoke',
    },
  ],
};
...
const workflow = {
  ...
  schema_version: '1.0.0',
  id: 'callweave.evidence-retain-test.workflow',
  name: 'evidence-retain-test',
  version: '0.1.0',
  lifecycle: 'active',
  owner: {
    team: 'callweave',
  ...
  summary: 'Exercise the compatible evidence-retain component through the app registration path.',
  inputs: contract.inputs,
  outputs: contract.outputs,
  nodes: [
    {
      node_id: 'retain_evidence',
      capability_id: 'callweave.evidence-retain',
      capability_version: '0.1.0',
      input: {
  ...
  edges: [],
  start_node: 'retain_evidence',
  terminal_nodes: ['retain_evidence'],
  tags: ['callweave', 'compatible', 'host-adapter', 'evidence-retain'],
  governing_spec: '007-workflow-registry-traversal',
};
await writeFile(join(workflowDir, 'workflow.json'), stableJson(workflow));

const appManifest = {
  app_id: 'callweave.evidence-retain-test',
  version: appVersion,
  schema_version: '1.0.0',
  workspace_defaults: {
    workspace_id: 'callweave-evidence-retain-local',
...
await writeFile(
  ...
  [
    '# Callweave evidence-retain compatible test app',
    '',
    'This app bundle exercises the host-integrated compatible-mode component',
    'for `callweave.evidence-retain`. It validates and registers a thin wrapper',
    'around the governed retention decision contract.',
    '',
    'Generate these files with:',
    '',
    '```bash',
    'node scripts/generate_callweave_evidence_retain_app.mjs',
    '```',
  ].join('\n') + '\n',
);
