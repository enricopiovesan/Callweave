...
const appRoot = join(repoRoot, 'apps', 'callweave-coverage-assess');
const componentDir = join(appRoot, 'components', 'coverage-assess');
const workflowDir = join(appRoot, 'workflows', 'coverage-assess');
const contractPath = join(repoRoot, 'traverse', 'contracts', 'callweave', 'coverage-assess', 'contract.json');
...
const wrapper = {
  ...
  capability_id: 'callweave.coverage-assess',
  notes: 'Thin wrapper only. Host-owned evidence reference resolution and durable coverage report persistence remain outside the portable contract.',
};
...
const componentManifest = {
  component_id: 'callweave.coverage-assess-compatible-component',
  version: '0.1.0',
  ...
  validation_evidence: [
    {
      ...
      status: 'passed',
      produced_by: 'coverage_assess_compatible_smoke',
    },
  ],
};
...
const workflow = {
  ...
  schema_version: '1.0.0',
  id: 'callweave.coverage-assess-test.workflow',
  name: 'coverage-assess-test',
  version: '0.1.0',
  lifecycle: 'active',
  owner: {
    team: 'callweave',
  ...
  summary: 'Exercise the compatible coverage-assess component through the app registration path.',
  inputs: contract.inputs,
  outputs: contract.outputs,
  nodes: [
    {
      node_id: 'assess_coverage',
      capability_id: 'callweave.coverage-assess',
      capability_version: '0.1.0',
      input: {
  ...
  edges: [],
  start_node: 'assess_coverage',
  terminal_nodes: ['assess_coverage'],
  tags: ['callweave', 'compatible', 'host-adapter', 'coverage-assess'],
  governing_spec: '007-workflow-registry-traversal',
};
await writeFile(join(workflowDir, 'workflow.json'), stableJson(workflow));

const appManifest = {
  app_id: 'callweave.coverage-assess-test',
  version: appVersion,
  schema_version: '1.0.0',
  workspace_defaults: {
    workspace_id: 'callweave-coverage-assess-local',
...
await writeFile(
  ...
  [
    '# Callweave coverage-assess compatible test app',
    '',
    'This app bundle exercises the host-integrated compatible-mode component',
    'for `callweave.coverage-assess`. It validates and registers a thin wrapper',
    'around the durable coverage-report governance contract.',
    '',
    'Generate these files with:',
    '',
    '```bash',
    'node scripts/generate_callweave_coverage_assess_app.mjs',
    '```',
  ].join('\n') + '\n',
);
