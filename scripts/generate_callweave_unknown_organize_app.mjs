...
const appRoot = join(repoRoot, 'apps', 'callweave-unknown-organize');
const componentDir = join(appRoot, 'components', 'unknown-organize');
const workflowDir = join(appRoot, 'workflows', 'unknown-organize');
const contractPath = join(repoRoot, 'traverse', 'contracts', 'callweave', 'unknown-organize', 'contract.json');
...
const wrapper = {
  ...
  capability_id: 'callweave.unknown-organize',
  notes: 'Thin wrapper only. Host-owned unknown evidence lookup and append-only cluster state remain outside the portable contract.',
};
...
const componentManifest = {
  component_id: 'callweave.unknown-organize-compatible-component',
  version: '0.1.0',
  ...
  validation_evidence: [
    {
      ...
      status: 'passed',
      produced_by: 'unknown_organize_compatible_smoke',
    },
  ],
};
...
const workflow = {
  ...
  schema_version: '1.0.0',
  id: 'callweave.unknown-organize-test.workflow',
  name: 'unknown-organize-test',
  version: '0.1.0',
  lifecycle: 'active',
  owner: {
    team: 'callweave',
  ...
  summary: 'Exercise the compatible unknown-organize component through the app registration path.',
  inputs: contract.inputs,
  outputs: contract.outputs,
  nodes: [
    {
      node_id: 'organize_unknowns',
      capability_id: 'callweave.unknown-organize',
      capability_version: '0.1.0',
      input: {
  ...
  edges: [],
  start_node: 'organize_unknowns',
  terminal_nodes: ['organize_unknowns'],
  tags: ['callweave', 'compatible', 'host-adapter', 'unknown-organize'],
  governing_spec: '007-workflow-registry-traversal',
};
await writeFile(join(workflowDir, 'workflow.json'), stableJson(workflow));

const appManifest = {
  app_id: 'callweave.unknown-organize-test',
  version: appVersion,
  schema_version: '1.0.0',
  workspace_defaults: {
    workspace_id: 'callweave-unknown-organize-local',
...
await writeFile(
  ...
  [
    '# Callweave unknown-organize compatible test app',
    '',
    'This app bundle exercises the host-integrated compatible-mode component',
    'for `callweave.unknown-organize`. It validates and registers a thin wrapper',
    'around the governed local unknown-clustering contract.',
    '',
    'Generate these files with:',
    '',
    '```bash',
    'node scripts/generate_callweave_unknown_organize_app.mjs',
    '```',
  ].join('\n') + '\n',
);
