...
const appRoot = join(repoRoot, 'apps', 'callweave-acoustics-classify');
const componentDir = join(appRoot, 'components', 'acoustics-classify');
const workflowDir = join(appRoot, 'workflows', 'acoustics-classify');
const contractPath = join(repoRoot, 'traverse', 'contracts', 'callweave', 'acoustics-classify', 'contract.json');
...
const wrapper = {
  ...
  capability_id: 'callweave.acoustics-classify',
  notes: 'Thin wrapper only. Host-owned model runtime executes prepared-audio classification outside the portable contract.',
};
...
const componentManifest = {
  component_id: 'callweave.acoustics-classify-compatible-component',
  version: '0.1.0',
  ...
  validation_evidence: [
    {
      ...
      status: 'passed',
      produced_by: 'acoustics_classify_compatible_smoke',
    },
  ],
};
...
const workflow = {
  ...
  schema_version: '1.0.0',
  id: 'callweave.acoustics-classify-test.workflow',
  name: 'acoustics-classify-test',
  version: '0.1.0',
  lifecycle: 'active',
  owner: {
    team: 'callweave',
  ...
  summary: 'Exercise the compatible acoustics-classify component through the app registration path.',
  inputs: contract.inputs,
  outputs: contract.outputs,
  nodes: [
    {
      node_id: 'classify_acoustics',
      capability_id: 'callweave.acoustics-classify',
      capability_version: '0.1.0',
      input: {
  ...
  edges: [],
  start_node: 'classify_acoustics',
  terminal_nodes: ['classify_acoustics'],
  tags: ['callweave', 'compatible', 'host-adapter', 'acoustics-classify'],
  governing_spec: '007-workflow-registry-traversal',
};
await writeFile(join(workflowDir, 'workflow.json'), stableJson(workflow));

const appManifest = {
  app_id: 'callweave.acoustics-classify-test',
  version: appVersion,
  schema_version: '1.0.0',
  workspace_defaults: {
    workspace_id: 'callweave-acoustics-classify-local',
...
await writeFile(
  ...
  [
    '# Callweave acoustics-classify compatible test app',
    '',
    'This app bundle exercises the host-integrated compatible-mode component',
    'for `callweave.acoustics-classify`. It validates and registers a thin',
    'wrapper around the prepared-audio classification contract.',
    '',
    'Generate these files with:',
    '',
    '```bash',
    'node scripts/generate_callweave_acoustics_classify_app.mjs',
    '```',
  ].join('\n') + '\n',
);
