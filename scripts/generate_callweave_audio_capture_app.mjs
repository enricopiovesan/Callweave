...
const appRoot = join(repoRoot, 'apps', 'callweave-audio-capture');
const componentDir = join(appRoot, 'components', 'audio-capture');
const workflowDir = join(appRoot, 'workflows', 'audio-capture');
const contractPath = join(repoRoot, 'traverse', 'contracts', 'callweave', 'audio-capture', 'contract.json');
...
const wrapper = {
  ...
  capability_id: 'callweave.audio-capture',
  notes: 'Thin wrapper only. Host-owned capture, finalization, and storage remain outside the portable contract.',
};
...
const componentManifest = {
  component_id: 'callweave.audio-capture-compatible-component',
  version: '0.1.0',
  ...
  validation_evidence: [
    {
      ...
      status: 'passed',
      produced_by: 'audio_capture_compatible_smoke',
    },
  ],
};
...
const workflow = {
  ...
  schema_version: '1.0.0',
  id: 'callweave.audio-capture-test.workflow',
  ...
  summary: 'Exercise the compatible audio-capture component through the app registration path.',
  inputs: contract.inputs,
  outputs: contract.outputs,
  nodes: [
    {
      node_id: 'capture_audio',
      capability_id: 'callweave.audio-capture',
      capability_version: '0.1.0',
      input: {
  ...
  edges: [],
  start_node: 'capture_audio',
  terminal_nodes: ['capture_audio'],
  tags: ['callweave', 'compatible', 'host-adapter', 'audio-capture'],
  governing_spec: '007-workflow-registry-traversal',
};
await writeFile(join(workflowDir, 'workflow.json'), stableJson(workflow));

const appManifest = {
  app_id: 'callweave.audio-capture-test',
  version: appVersion,
  schema_version: '1.0.0',
  workspace_defaults: {
    workspace_id: 'callweave-audio-capture-local',
...
await writeFile(
  ...
  [
    '# Callweave audio-capture compatible test app',
    '',
    'This app bundle exercises the host-integrated compatible-mode component',
    'for `callweave.audio-capture`. It validates and registers a thin wrapper',
    'around the recording capture contract.',
    '',
    'Generate these files with:',
    '',
    '```bash',
    'node scripts/generate_callweave_audio_capture_app.mjs',
    '```',
  ].join('\n') + '\n',
);
