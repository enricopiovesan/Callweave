...
const appRoot = join(repoRoot, 'apps', 'callweave-daily-close');
const componentDir = join(appRoot, 'components', 'daily-close');
const workflowDir = join(appRoot, 'workflows', 'daily-close');
const contractPath = join(repoRoot, 'traverse', 'contracts', 'callweave', 'daily-close', 'contract.json');
...
const wrapper = {
  ...
  capability_id: 'callweave.daily-close',
  notes: 'Thin wrapper only. Host-owned scheduling, local-day resolution, and append-only close persistence remain outside the portable contract.',
};
...
const componentManifest = {
  component_id: 'callweave.daily-close-compatible-component',
  version: '0.1.0',
  ...
  validation_evidence: [
    {
      ...
      status: 'passed',
      produced_by: 'daily_close_compatible_smoke',
    },
  ],
};
...
const workflow = {
  ...
  schema_version: '1.0.0',
  id: 'callweave.daily-close-test.workflow',
  name: 'daily-close-test',
  version: '0.1.0',
  lifecycle: 'active',
  owner: {
    team: 'callweave',
  ...
  summary: 'Exercise the compatible daily-close component through the app registration path.',
  ...
  edges: [],
  start_node: 'close_day',
  terminal_nodes: ['close_day'],
  tags: ['callweave', 'compatible', 'host-adapter', 'daily-close'],
  governing_spec: '007-workflow-registry-traversal',
};
await writeFile(join(workflowDir, 'workflow.json'), stableJson(workflow));

const appManifest = {
  app_id: 'callweave.daily-close-test',
  version: appVersion,
  schema_version: '1.0.0',
  workspace_defaults: {
    workspace_id: 'callweave-daily-close-local',
...
await writeFile(
  ...
  [
    '# Callweave daily-close compatible test app',
    '',
    'This app bundle exercises the host-integrated compatible-mode component',
    'for `callweave.daily-close`. It validates and registers a thin wrapper',
    'around the append-only local-day close contract.',
    '',
    'Generate these files with:',
    '',
    '```bash',
    'node scripts/generate_callweave_daily_close_app.mjs',
    '```',
  ].join('\n') + '\n',
);
