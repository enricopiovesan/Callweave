...
const appRoot = join(repoRoot, 'apps', 'callweave-daily-revise');
const componentDir = join(appRoot, 'components', 'daily-revise');
const workflowDir = join(appRoot, 'workflows', 'daily-revise');
const contractPath = join(repoRoot, 'traverse', 'contracts', 'callweave', 'daily-revise', 'contract.json');
...
const wrapper = {
  ...
  capability_id: 'callweave.daily-revise',
  notes: 'Thin wrapper only. Host-owned artifact persistence and append-only revision history remain outside the portable contract.',
};
...
const componentManifest = {
  component_id: 'callweave.daily-revise-compatible-component',
  version: '0.1.0',
  ...
  validation_evidence: [
    {
      ...
      status: 'passed',
      produced_by: 'daily_revise_compatible_smoke',
    },
  ],
};
...
const workflow = {
  ...
  schema_version: '1.0.0',
  id: 'callweave.daily-revise-test.workflow',
  name: 'daily-revise-test',
  version: '0.1.0',
  lifecycle: 'active',
  owner: {
    team: 'callweave',
  ...
  summary: 'Exercise the compatible daily-revise component through the app registration path.',
  inputs: contract.inputs,
  outputs: contract.outputs,
  nodes: [
    {
      node_id: 'revise_daily_canvas',
      capability_id: 'callweave.daily-revise',
      capability_version: '0.1.0',
      input: {
  ...
  edges: [],
  start_node: 'revise_daily_canvas',
  terminal_nodes: ['revise_daily_canvas'],
  tags: ['callweave', 'compatible', 'host-adapter', 'daily-revise'],
  governing_spec: '007-workflow-registry-traversal',
};
await writeFile(join(workflowDir, 'workflow.json'), stableJson(workflow));

const appManifest = {
  app_id: 'callweave.daily-revise-test',
  version: appVersion,
  schema_version: '1.0.0',
  workspace_defaults: {
    workspace_id: 'callweave-daily-revise-local',
...
await writeFile(
  ...
  [
    '# Callweave daily-revise compatible test app',
    '',
    'This app bundle exercises the host-integrated compatible-mode component',
    'for `callweave.daily-revise`. It validates and registers a thin wrapper',
    'around the immutable daily canvas revision contract.',
    '',
    'Generate these files with:',
    '',
    '```bash',
    'node scripts/generate_callweave_daily_revise_app.mjs',
    '```',
  ].join('\n') + '\n',
);
