...
const appRoot = join(repoRoot, 'apps', 'callweave-review-prepare');
const componentDir = join(appRoot, 'components', 'review-prepare');
const workflowDir = join(appRoot, 'workflows', 'review-prepare');
const contractPath = join(repoRoot, 'traverse', 'contracts', 'callweave', 'review-prepare', 'contract.json');
...
const wrapper = {
  ...
  capability_id: 'callweave.review-prepare',
  notes: 'Thin wrapper only. Host-owned sanitized bundle creation, external advisory response ingress, and append-only review proposal persistence remain outside the portable contract.',
};
...
const componentManifest = {
  component_id: 'callweave.review-prepare-compatible-component',
  version: '0.1.0',
  ...
  validation_evidence: [
    {
      ...
      status: 'passed',
      produced_by: 'review_prepare_compatible_smoke',
    },
  ],
};
...
const workflow = {
  ...
  schema_version: '1.0.0',
  id: 'callweave.review-prepare-test.workflow',
  name: 'review-prepare-test',
  version: '0.1.0',
  lifecycle: 'active',
  owner: {
    team: 'callweave',
  ...
  summary: 'Exercise the compatible review-prepare component through the app registration path.',
  ...
  edges: [],
  start_node: 'prepare_review',
  terminal_nodes: ['prepare_review'],
  tags: ['callweave', 'compatible', 'host-adapter', 'review-prepare'],
  governing_spec: '007-workflow-registry-traversal',
};
await writeFile(join(workflowDir, 'workflow.json'), stableJson(workflow));

const appManifest = {
  app_id: 'callweave.review-prepare-test',
  version: appVersion,
  schema_version: '1.0.0',
  workspace_defaults: {
    workspace_id: 'callweave-review-prepare-local',
...
await writeFile(
  ...
  [
    '# Callweave review-prepare compatible test app',
    '',
    'This app bundle exercises the host-integrated compatible-mode component',
    'for `callweave.review-prepare`. It validates and registers a thin',
    'wrapper around the governed advisory review app contract.',
    '',
    'Generate these files with:',
    '',
    '```bash',
    'node scripts/generate_callweave_review_prepare_app.mjs',
    '```',
  ].join('\n') + '\n',
);
