...
const appRoot = join(repoRoot, 'apps', 'callweave-observation-manage');
const componentDir = join(appRoot, 'components', 'observation-manage');
const workflowDir = join(appRoot, 'workflows', 'observation-manage');
const contractPath = join(repoRoot, 'traverse', 'contracts', 'callweave', 'observation-manage', 'contract.json');
...
const wrapper = {
  ...
  capability_id: 'callweave.observation-manage',
  notes: 'Thin wrapper only. Host-owned append-only observation history and review-governed supersession remain outside the portable contract.',
};
...
const componentManifest = {
  component_id: 'callweave.observation-manage-compatible-component',
  version: '0.1.0',
  ...
  validation_evidence: [
    {
      ...
      status: 'passed',
      produced_by: 'observation_manage_compatible_smoke',
    },
  ],
};
...
const workflow = {
  ...
  schema_version: '1.0.0',
  id: 'callweave.observation-manage-test.workflow',
  name: 'observation-manage-test',
  version: '0.1.0',
  lifecycle: 'active',
  owner: {
    team: 'callweave',
  ...
  summary: 'Exercise the compatible observation-manage component through the app registration path.',
  inputs: contract.inputs,
  outputs: contract.outputs,
  nodes: [
    {
      node_id: 'manage_observation',
      capability_id: 'callweave.observation-manage',
      capability_version: '0.1.0',
      input: {
  ...
  edges: [],
  start_node: 'manage_observation',
  terminal_nodes: ['manage_observation'],
  tags: ['callweave', 'compatible', 'host-adapter', 'observation-manage'],
  governing_spec: '007-workflow-registry-traversal',
};
await writeFile(join(workflowDir, 'workflow.json'), stableJson(workflow));

const appManifest = {
  app_id: 'callweave.observation-manage-test',
  version: appVersion,
  schema_version: '1.0.0',
  workspace_defaults: {
    workspace_id: 'callweave-observation-manage-local',
...
await writeFile(
  ...
  [
    '# Callweave observation-manage compatible test app',
    '',
    'This app bundle exercises the host-integrated compatible-mode component',
    'for `callweave.observation-manage`. It validates and registers a thin wrapper',
    'around the versioned observation governance contract.',
    '',
    'Generate these files with:',
    '',
    '```bash',
    'node scripts/generate_callweave_observation_manage_app.mjs',
    '```',
  ].join('\n') + '\n',
);
